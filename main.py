...
from lib.import_export_data import (
    send_request_to_tally,
    send_data_to_evitalrx,
    reset_mapping_from_rx,
    check_if_tally_running,
    send_init_data_to_evital_rx,
)
...
spinner_loader = itertools.cycle(
    ["⬤       ", "     ⬤  ", "       ⬤", "  ⬤     "]
)  # Larger symbols for circular pattern
...
def main_thread():
    ...
    list_of_dates = [
        x["synced_timestamp"]
        for x in constants.LOGIN_RESPONSE["data"]["last_synced_history"]
    ]
    ...
    select_software_frame = Frame(
        root,
        bg="#75a8f0",
        highlightbackground="#280754",
        highlightcolor="#280754",
        highlightthickness=1,
    )
    select_software_frame.pack(side="top", pady=10, padx=20, fill="x")

    select_software_label = Label(
        select_software_frame,
    ...
    select_software_dropdown = Combobox(
        select_software_frame, width=30, textvariable=combo_box_text
    )
    ...
    additional_message_label = Label(
        additional_message_frame, text="", font=("Helvetica", 10), bg="#F0F0F0"
    )
    ...
    def start_thread(start_now=False):
        ...
        if constants.THREAD is None:
            background_thread = threading.Thread(
                target=background_sync, args=(start_now,), daemon=True
            )
    ...
    def startprocess():
        ...
        to_date = date(int(str(select_software_dropdown.get()).split("-")[1]), 3, 31)
        ...
        print(
            "➡ main.py:168 constants.DISPLAY_SYNC_LOADER:",
            constants.DISPLAY_SYNC_LOADER,
        )
        ...
        if constants.THREAD is None:
            messagebox.showinfo(
                "Tally Data Export",
                str(res["status_message"]).replace("_", " ").title(),
            )
    ...
    get_files_button = Button(
        ...
        command=lambda: start_thread(True),
    )
