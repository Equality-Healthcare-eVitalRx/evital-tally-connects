...
def login_thread():
    ...
    def show_mapping_screen(account_window):
        ...
        label_instruction = tk.Label(
            mapping_window,
            text="Map your eVital accounts with unique Tally Companies:",
            font=("Helvetica", 14, "bold"),
            bg="#F0F0F0",
        )
        label_instruction.bind(
            "<Configure>",
            lambda e: label_instruction.config(
                wraplength=label_instruction.winfo_width()
            ),
        )
        label_instruction.pack(pady=30)

        accounts = [
            x["entity_business_name"] for x in constants.RX_ACCOUNTS
        ]  # Example accounts
        categories = [
        ...
        def update_options():
            ...
            for account, combobox in mapping_entries:
                ...
                combobox["values"] = available_categories + (
                    [current_selection] if current_selection else []
                )
        ...
        scrollbar = ttk.Scrollbar(
            mapping_window, orient="vertical", command=canvas.yview
        )
        ...
        label_account = tk.Label(
            header_frame, text="eVital Accounts", font=("Helvetica", 12), bg="#F0F0F0"
        )
        label_account.pack(side=tk.LEFT, padx=20)
        label_tally = tk.Label(
            header_frame, text="Tally Companies", font=("Helvetica", 12), bg="#F0F0F0"
        )
        ...
        for account in accounts:
            ...
            label_account = tk.Label(
                frame, text=account + ":", font=("Helvetica", 12), bg="#F0F0F0"
            )
            label_account.pack(side=tk.LEFT, padx=20)
            ...
            combobox.bind(
                "<<ComboboxSelected>>",
                lambda event, a=account: category_selected(event, a),
            )
        ...
        def submit_mapping(account_window):
            ...
            constants.COMPANY_MAPPING = [
                {
                    "chemist_id": x["id"],
                    "company_name": combobox.get(),
                    "company_guid": y["company_guid"],
                    "mapping_type": "multiple",
                }
                for account, combobox in mapping_entries
                for x in constants.RX_ACCOUNTS
                if x["entity_business_name"] == account
                for y in constants.TALLY_ACCOUNTS
                if y["company_name"] == combobox.get()
            ]
            ...
            LogManagerObj.write_log(
                "company mapping : " + str(constants.COMPANY_MAPPING)
            )
            if len(constants.COMPANY_MAPPING) < 1:
                messagebox.showinfo(
                    "Map Tally Companies", "Please select atleast one company"
                )
            else:
                res = map_rx_companies()
                if res not in [0] and (
                    "status_code" in res.keys()
                    and res["status_code"] in [1, "1", "1.0"]
                ):
                    ...
                    messagebox.showerror(
                        "Map Companies", "Error while mapping companies."
                    )
        ...
        button_submit = tk.Button(
            ...
            bg="#4CAF50",
            fg="white",
            command=lambda a=account_window: submit_mapping(a),
        )
    ...
    def show_single_account_selection(account_window):
        ...
        label_instruction = tk.Label(
            single_account_window,
            text="Select your single Tally Company:",
            font=("Helvetica", 14, "bold"),
            bg="#F0F0F0",
        )
        ...
        def submit_single_account():
            ...
            if constants.LOGIN_RESPONSE["data"]["business_details"][
                ...
                constants.COMPANY_MAPPING = [
                    {
                        "chemist_id": x["id"],
                        "company_name": combobox_account.get(),
                        "company_guid": y["company_guid"],
                        "mapping_type": "single",
                    }
                    for x in constants.RX_ACCOUNTS
                    if x["entity_business_name"]
                    == constants.LOGIN_RESPONSE["data"]["business_details"][
                        "logged_in_business"
                    ]["entity_business_name"]
                    for y in constants.TALLY_ACCOUNTS
                    if y["company_name"] == combobox_account.get()
                ]
                # else:
            ...
            if res not in [0] and (
                "status_code" in res.keys() and res["status_code"] in [1, "1", "1.0"]
            ):
                # with open("./lib/app_cache.txt") as data_file:
                #     data = json.load(data_file)
    ...
    def ask_account_type():
        ...
        label_question = tk.Label(
            sub_panel,
            text="Do you have a single Tally Company for all your evital Accounts?",
            wraplength=400,
        ...
        button_single = tk.Button(
            ...
            fg="white",
            width=20,
            command=lambda: account_selected("Single", account_window),
        )
        button_single.pack(pady=10, padx=10)

        button_multiple = tk.Button(
            sub_panel,
            ...
            fg="white",
            width=20,
            command=lambda: account_selected("Multiple", account_window),
        )
    ...
    def login():
        ...
        if len(mobile_number) != 10 and str(mobile_number).isdigit() == False:
            ...
            if "status_code" in res.keys() and res["status_code"] in [1, "1"]:
                ...
                # print(res["data"])
                constants.COMPANY_MAPPING = res["data"]["business_details"][
                    "company_mapping_details"
                ]
                ...
                data = {
                    ...
                    "company_mapping": res["data"]["business_details"][
                        "company_mapping_details"
                    ],
                }
                # with open("./lib/app_cache.txt", "w") as json_file:
                ...
                already_mapped = (
                    True
                    if len(res["data"]["business_details"]["company_mapping_details"])
                    > 0
                    else False
                )
                if already_mapped:
                    ...
                    if if_chain_pharmacy:
                        # print()
                        # if if_chain_pharmacy:
                        if_ho = res["data"]["business_details"]["logged_in_business"][
                            "is_HO"
                        ]
                        if if_ho:
                            ask_account_type()
    ...
    mobile_label = tk.Label(
        right_panel, text="Mobile Number", bg="white", fg="#044C9D", font=header_font1
    )
    ...
    password_entry = tk.Entry(
        right_panel, bg="white", font=header_font1, bd=0, show="*"
    )
