...
2. The version is compared against `constants.APP_VERSION` (read from the bundled `VERSION` file — the single source of truth). `build.py` also regenerates `version.txt` (the Windows executable FileVersion metadata) from `VERSION` before each build.
...
├── VERSION                     # Single source of truth for app version (e.g. 3.11.1), bundled
├── version.txt                 # PyInstaller Windows version metadata — Auto-generated from VERSION by build.py
...
> **Note:** The `VERSION` file is the **single source of truth** for the app version. `build.py` reads it and **automatically regenerates `version.txt`** (the PyInstaller Windows FileVersion metadata) from it before each build, so they never drift apart. Just bump `VERSION` when cutting a new release — no need to edit `version.txt` manually.
